...
- Heavy uncertainty around the eventual _real_ domain boundaries (creators, fans, payments, ledger, etc.).
...
- Every domain lives in its own folder under `apps/api/src/modules/<domain>/` with a consistent inner shape (`controllers`, `services`, `repositories` _only when owned_, `validators`, `routes`, `types`, `tests`).
...
- **Single source of truth for types.** `@universal-healthcare/shared` types are imported freely across modules — there is _one_ `User`, _one_ `CreatorProfile`, _one_ `FanProfile`.
...
- **Scaling shape is vertical.** When one module becomes 10× heavier than the others, the answer is "buy a bigger box" until we explicitly split it out (see _Mitigation_).
