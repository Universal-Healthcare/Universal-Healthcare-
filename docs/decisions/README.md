...
> Why we did what we did — short records that capture a decision's _context_, the choice we made, and the _consequences_ (good and bad). Future contributors should be able to read these to understand the shape of the codebase, not just re-discover it.
...
4. **Consequences** — the _positive_ outcomes we gained followed by the _negative_ outcomes we accepted (with mitigations when we have them).
...
- Past decisions are **immutable** — if we change our mind, we _supersede_ the old ADR rather than editing it
...
- **Don't editorialize the past.** Record _why the choice was rational at the time_, not whether it was a mistake.
...
When the codebase grows, this folder should be the _first_ thing you read on day one. If it isn't, we wrote the wrong ones.
